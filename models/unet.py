...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if use_spatial_transformer:
            assert context_dim is not None, 'Oops... You forgot to include the dimension of your cross-attention conditioning...'

        if context_dim is not None:
            assert use_spatial_transformer, 'Oops... You forgot to use the spatial transformer for your cross-attention conditioning...'

        if num_heads_upsample == -1:
            num_heads_upsample = num_heads

        self.sample_size = sample_size  # (H, W, D)
    ...
    def forward(
        ...
        **kwargs):
        ...
        assert timestep is not None, 'need to implement no-timestep usage'

        # NOTE: construction of conditioning is moved here
        if isinstance(encoder_hidden_states, dict):
            c_concat = encoder_hidden_states['c_concat']
            c_crossattn = encoder_hidden_states['c_crossattn']
        else:
            latent_shape = sample.shape
            cond = encoder_hidden_states  # shape: [b, 4]
            c_concat = cond.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            c_concat = c_concat.expand(list(cond.shape[:2]) + list(latent_shape[2:]))
            c_crossattn = cond.unsqueeze(1)
        
        # NOTE: concat is moved here
        x = torch.cat([sample, c_concat], dim=1)
        context = c_crossattn
